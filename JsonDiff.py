    ...
    def check(self, first, second, path='', with_values=False):
        if second != None:
            if not isinstance(first, type(second)):
                ...
                self.save_diff(message, self.TYPE)
            ...
            for key in first:
                ...
                if isinstance(second, dict):
                    if second.has_key(key):
                        ...
                        self.save_diff(new_path, self.PATH)
            ...
            for (index, item) in enumerate(first):
                ...
                if second != None:
                    try:
                        ...
                        # goes to difference
                        self.save_diff('%s - %s, %s' % (new_path, type(first), type(second)), self.TYPE)
